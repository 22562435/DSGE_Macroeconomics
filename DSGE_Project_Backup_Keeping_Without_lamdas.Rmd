...
In this expression, $mc_t = \tfrac{\mathrm{MC}_t}{P_t}$ denotes the real marginal cost, and $\tfrac{\epsilon-1}{\epsilon}$ is its flexible‐price benchmark. The left‐hand side captures current inflation’s departure from unity, while the first term on the right relates that departure to real marginal cost gaps scaled by the adjustment‐cost parameter $\psi$. A larger $\psi$ makes prices stickier, damping the response of inflation to cost pressures. The second term brings in expected future inflation, discounted by $\beta$, adjusted for households’ intertemporal marginal‐utility ratio $\lambda_{t+1}/\lambda_t$ and relative output growth $Y_{t+1}/Y_t$. Together, these components link today’s inflation dynamics to both real‐economic conditions and expectations of tomorrow’s price adjustments.
