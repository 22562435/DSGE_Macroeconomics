...
This two-tiered structure captures core New Keynesian dynamics, such as price stickiness and strategic pricing, while retaining analytical tractability.
...
**(Monopolistically competitive firms,  with nominal rigidities)**
...
Intermediate firms set prices subject to Rotemberg-style adjustment costs. The firm chooses a price path to maximize the expected discounted sum of profits, which includes a quadratic penalty for deviating from past prices. Imposing symmetry and using first-order conditions, we obtain the pricing equation:
...
Equation \eqref{rotemberg_FOC} is a New Keynesian Phillips Curve under Rotemberg pricing. It links inflation dynamics $\pi_t$ to real marginal costs $mc_t$ and expected future inflation. The parameter $\psi$ governs price rigidity: higher values imply stronger penalties for price changes and thus more persistent inflation dynamics.
...
- \(mc_t = \frac{\text{MC}_t}{P_t}\) = Real marginal cost (eq \eqref{marginal_cost})
- \(Y_t(j) = \left( \frac{P_t(j)}{P_t} \right)^{-\epsilon} Y_t\) (demand curve from eq \eqref{demand_curve_final})
...
Intermediate goods producers face costs when adjusting their prices, following the Rotemberg (1982) framework. These adjustment costs are quadratic in the inflation rate, meaning firms pay increasingly higher costs for larger price changes. The functional form captures the economic intuition that rapid price adjustments are more disruptive and costly than gradual ones.

When setting prices, each firm balances three key considerations. First, higher prices generate more revenue per unit sold. Second, production becomes more expensive as output increases due to rising marginal costs. Third, changing prices itself carries direct costs. The firm's optimization problem involves choosing a price path that maximizes the present discounted value of profits while accounting for these competing factors.

In symmetric equilibrium where all firms behave identically, this price-setting behavior aggregates to yield the Rotemberg Phillips Curve. This fundamental equation relates current inflation to real marginal costs and expected future inflation. The specific form emerges from the first-order conditions of the firm's optimization problem, with the quadratic adjustment costs creating a nonlinear relationship.

The Phillips Curve reveals several important economic insights. Current inflation responds positively to deviations of real marginal costs from their flexible-price equilibrium level, which in this model equals (ε-1)/ε where ε represents the elasticity of substitution between goods. Inflation also depends on expected future inflation, with the strength of this forward-looking component determined by the household discount factor β. The sensitivity of inflation to these factors is inversely related to the price adjustment cost parameter ψ - when price changes are more costly (higher ψ), inflation becomes less responsive to economic conditions.

The equation captures essential New Keynesian characteristics of inflation dynamics. It embodies the forward-looking nature of price setting while maintaining the principle that inflation ultimately reflects real economic conditions through the marginal cost channel. The specific functional form arises directly from the Rotemberg adjustment cost specification, providing microeconomic foundations for the inflation process that are consistent with the rest of the model's optimization-based structure.
...
Because $\tau_t = T_t/P_t$, this fully pins down the lump‐sum tax rate each period (once you know the price level $P_t$).  In your code you would simply treat $T_t$ as an exogenous state whose only “choice” is driven by the AR(1) shock $\varepsilon_t^T$.
