...
dynareCodes_NKM <- '
/* ==== Partial New‑Keynesian DSGE ====*/

var c C i I M B K h Rb Rk W P Pi tau q Y Yj Pj MC A Kj hj;
varexo eps_a eps_m;
...
model;
  ...
  # labour supply
  W/P = chi*h^gamma / ((c-eta*c(-1))^(-theta) - beta*eta*E((c(+1)-eta*c)^(-theta)));

  # money demand
  M = psi / (beta*E((Rb-1)*((c(+1)-eta*c)^(-theta) - beta*eta*E((c(+2)-eta*c(+1))^(-theta)))/P(+1)));

  # production: final goods
  Y  = (integral(Yj^((epsilon-1)/epsilon), j, 0, 1))^(epsilon/(epsilon-1));
  Yj = (Pj/P)^(-epsilon)*Y;
  P  = (integral(Pj^(1-epsilon), j, 0, 1))^(1/(1-epsilon));

  # production: intermediate goods & marginal cost
